    ...
    <main className="sist-command-site">
      ...
      <section className="method-panel">
        <div className="method-header">
          ...
          <p className="method-sub">Challenge every assumption. Deconstruct every claim. Synthesize what survives into an accountable strategic record.</p>
        </div>
        ...
        <div className="division-grid">
          {divisions.map(([name, copy, icon, href]) => (
            <Link href={href} className="division-card" key={name}>
              ...
              <h3>{name}</h3>
              <p>{copy}</p>
              <span className="division-cta">ENTER DIVISION <i aria-hidden="true">→</i></span>
            </Link>
          ))}
        </div>
      </section>
